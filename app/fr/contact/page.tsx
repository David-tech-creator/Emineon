...
export default function ContactPage() {
  ...
  function validate() {
    ...
    else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(form.email)) errs.email = "Veuillez saisir un e-mail valide.";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setApiError(data.error || "Échec de l'envoi du message.");
        setLoading(false);
        return;
      }
      setSubmitted(true);
  ...
  return (
    <main className="min-h-screen w-full flex flex-col items-center justify-center px-2 py-6 sm:py-10 relative overflow-hidden">
      ...
      >
        <div className="flex w-full min-h-screen items-center justify-center">
          <div className="bg-white/30 rounded-xl p-2 sm:p-4 max-w-md w-full flex flex-col items-center backdrop-blur-sm border border-white/30 relative">
            ...
            <h1 className="text-2xl font-bold text-emineon-blue mb-1 text-center">Contactez-nous</h1>
            {submitted ? (
              <motion.div
              ...
              >
                ...
                <div className="text-neutral-700">Nous avons bien reçu votre message et nous vous répondrons rapidement.</div>
              </motion.div>
            ) : (
              <form className="space-y-5 w-full" onSubmit={handleSubmit} noValidate>
                <div>
                  <label htmlFor="name" className="block text-sm font-medium text-emineon-blue mb-1">Nom *</label>
                  <motion.input
                ...
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-emineon-blue mb-1">E-mail *</label>
                  <motion.input
                    whileFocus={{ borderColor: "#C75B12" }}
                ...
                <div>
                  <label htmlFor="company" className="block text-sm font-medium text-emineon-blue mb-1">Entreprise</label>
                  <motion.input
                    whileFocus={{ borderColor: "#C75B12" }}
      ...
      <footer className="w-full flex flex-col items-center mt-8 mb-2 relative z-10">
        ...
        <span className="text-xs text-neutral-500 mt-2">© {new Date().getFullYear()} EMINEON. Tous droits réservés.</span>
      </footer>
    </main>
  );
} 